/* ==============================================
   Pixl Forge — Landing Hub Scripts
...
(function () {
  ...
  document.querySelectorAll('[data-track]').forEach((el) => {
    el.addEventListener('click', function () {
      ...
      const payload = { platform, product, href, link_text: (el.textContent || '').trim().slice(0, 60), page_section: getSection(el) };
  ...
  function onScroll() {
    ...
    milestones.forEach((m) => { if (pct >= m && !fired.has(m)) { fired.add(m); track('scroll_depth', { depth: m }); } });
  }
  let scrollTimeout;
  window.addEventListener('scroll', function () {
    if (scrollTimeout) return;
    scrollTimeout = setTimeout(() => { onScroll(); scrollTimeout = null; }, 250);
  }, { passive: true });
  ...
  setInterval(() => {
    ...
    timeOnPage += 5;
    heartbeats.forEach((s) => { if (timeOnPage >= s && !beatFired.has(s)) { beatFired.add(s); track('time_on_page', { seconds: s }); } });
  }, 5000);
  ...
  if ('IntersectionObserver' in window) {
    ...
    document.querySelectorAll('.platform-card, .product, .quote').forEach((el) => observer.observe(el));
  }
